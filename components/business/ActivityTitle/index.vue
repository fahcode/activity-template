<style lang="less">
    .activity-title {
        ...
        &__circle {
            ...
            width: 0.75rem; // 81px
            height: 0.75rem;
            background-image: url('../../assets/title-circle.png');
...
<script>
    ...
    export default {
        ...
        methods: {
            goBack() {
                /* if(util2.getUrlParam("fromswitch")){
                    // 统计从哪里返回到哪里
                    logger.makeActivityLog("activity_switch_titleback", {
                        fromid: util2.getUrlParam("toid") || '', 
                        toid: util2.getUrlParam("fromid") || '',
                        toname: util2.getUrlParam("fromname") || ''
                    });
                    window.history.go(-1);
                    return;
                } */

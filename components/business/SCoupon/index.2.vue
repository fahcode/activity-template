...
<script>
    ...
    export default {
        ...
        computed: {
            appList(){
                ...
                let apps = [],
                    ...
                    // index = 0;
                // while(true) {
                    apps = util.getApps(this.map.length);//util.getApps(4,index);
                    if(!apps.length) {
                        // break;
                        return []
                    }
                    ...
                    let arr [],
                        alen = apps.length,
                        len = this.map.length - alen,
                        i = 0;
                    for(i = 0; i < len.length; i++){
                        arr.push(apps[i % alen]);
                    }
                    // if(apps.length < 4) {
        ...
        methods:{
                ...
                NativeInterface.on(Enum.INTERFACE_EVENT.APP_PROGRESS, (pkg, btnText, textColor, bgColor, clickEnable, isInstalled)=> {
                    this.normalizeApps.forEach((app,index) => {
                        if(app.packageName == pkg) {
                            self.updateAppInfo(index,'progress',btnText);
                ...
                NativeInterface.on(Enum.INTERFACE_EVENT.APP_INSTALLED, (pkg) => {
                    this.normalizeApps.forEach((app,index) => {
                        if(app.packageName == pkg) {
                            ...
                            self.updateAppInfo(index,'installed');
                            // let oldStatus = app._status;
                            // app._btnImg = self.btn.url
            ...
            updateAppInfo(index,status = '',text = ''){
                let apps = this.yapps,
                    one = this.map[index],
                    app = apps[index],
                    progress = this.$refs['app__progress--'+index][0],
                    isInstalled = apps && app && NativeInterface.isAppInstalled(app.packageName),
                    isgoing = /^[\d]+/g.test(one.btnTxt) || one.btnTxt == '继续',
                    times = one.times,
                    chance = times,
                    btnTxt = this.ungetText,
                    couponStatus = 0,
                    ...
                    _btnImg = app._btnImg;
                if(times > 0){  // 有次数
                    if(one.use == one.total){ // 有次数，但是已抢光
                        if(isInstalled){  // 有次数，但是已抢光，并且已安装
                            btnTxt = this.cleanText
                            couponStatus = -1
                        }else if(isgoing){  // 有次数，已抢光，当前未安装，并且当前正在安装中
                            downloadProgress = true;
                        }else { // 有次数，已抢光，但是未安装（进行再次安装）
                            btnTxt = '下载游戏'
                        }
                    }else if(!isInstalled) {  // 有次数，未抢光，并且当前未安装
                        if(isgoing){  // 有次数，未抢光，当前未安装，并且当前正在安装中
                            downloadProgress = true;
                        }
                    }
                    // 有次数，未抢光，已安装，并且当前未领取优惠券
                }else if(!isInstalled){ // 无次数，未下载
                    if(one.use == one.total){  // 无次数，未下载，并且已抢光
                        if(isgoing){ // 下载中
                            downloadProgress = true;
                        }else{
                            btnTxt = '下载游戏'
                        }
                    }else if(isgoing)){  // 无次数，未抢光，并且正在下载中
                        downloadProgress = true;
                    }
                    chance = 0
                }else{  // 无次数，已下载
                    chance = 0
                    if(one.use == one.total){
                        btnTxt = this.cleanText
                        couponStatus = -1
                    }else{
                        btnTxt = this.gotText
                    ...
                    // if(util.getStorage('zippo_' + one.zippoId) == '3'){
                    //     btnTxt = this.cleanText
                    //     couponStatus = -1
                    // }else {
                    //     btnTxt = this.gotText
                ...
                if(status == 'progress'){  // 下载中
                    if(text == '安装') {
                        opacity = 0;
                        
                    }else if(text == '打开'){
                        opacity = 0;
                        _btnImg = this.btn.url
                    }else {
                        opacity = 1;
                        _btnImg = this.downloading.url;
                        btnTxt = text;
                        ...
                        if(num){
                            backgroundPosition = this.$toRem(191 * (num / 100 - 1))+' 0'
                        }
                        if(text == '正在安装' || text == '等待安装'){
                            backgroundPosition = '0 0'
                        }
                    }
                }else if(status == 'installed'){ // 已下载
                    let oldStatus = app._status;
                    _btnImg = this.btn.url
                    app._status = util.getAppStatus(app); //获取初始状态
                    if(app.downloadProgress){
                        if(this.apktxt[app.packageName]){
                            btnTxt = this.apktxt[app.packageName]
                        }else if(one.chance){
                            btnTxt = this.ungetText;
                        }
                    }else{
                    ...
                    if((app.type == 'INSTALL' || app.type == 'INSTALL_G')) { //安装类且仍未获得抽奖机会  //  && app._status != Enum.APP_STATUS.GOT && !util.getStorage('task_' + app.taskId)
                        ...
                        if(this.installed_apps.indexOf(app.taskId) == -1){
                            this.installed_apps.push(app.taskId)
                            if(app.downloadProgress){
                                this.postTask()
                            }
                        }
                ...
                if(backgroundPosition !== '-1'){
                    progress.style.backgroundPosition = backgroundPosition;
                }
                this.txt = Date.now();
            },
            initSingleAppData(){

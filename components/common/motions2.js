        ...
        if(NativeInterface.getUserId()) {
            ...
            .then((response) => {
                ...
                myAwards.forEach((value) => {
                    ...
                    if(typeof map[value.award_id] != 'undefined') {
                        map[value.award_id].count++;
                        return;
                    } else {
                        let award = util.findAwardById(value.award_id);
                            award.code = value.code; //虚拟奖品需要code值
                        award = Vue.util.extend({count: 1, awardRecordId: value.id || 0}, award);
                        //需要填写领取的信息，要单独分离显示
                        if (!(
                            value.type == Enum.AWARD_TYPE.RATEOFFLOW || //流量
                            value.type == Enum.AWARD_TYPE.TELCHARGE || //话费
                            value.type == Enum.AWARD_TYPE.VIRTUAL //虚拟奖品
                        )) {
            ...
            if(type == Enum.AWARD_TYPE.TELCHARGE) {
                this.$store.dispatch('postCharge', { context: this, params: { awardId: awardId, awardType: type, awardRecordId: awardRecordId || 0}}).then(function(value) {
                    if(value) {
                        ...
                        util.setStorage('telPhone_' + value[0].awardId, Date.now());
